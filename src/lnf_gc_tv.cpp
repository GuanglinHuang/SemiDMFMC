...
double lnf_gc_tv(SEXP ZZZ, SEXP TTT, SEXP T0){
  ...
  for(int tt = 1; tt < TT; tt++) {
     ...
     if(zt1 > 0){
       theta1_t = t10 + t11*theta1_t1 + t121*(1 + t13*abs(zt1))*pow(zt1,3);
       theta2_t = t20 + t21*theta2_t1 + t221*(1 + t23*abs(zt1))*pow(zt1,4);
     }
     if(zt1 <= 0){
       theta1_t = t10 + t11*theta1_t1 + t122*(1 + t13*abs(zt1))*pow(zt1,3);
       theta2_t = t20 + t21*theta2_t1 + t222*(1 + t23*abs(zt1))*pow(zt1,4);
     }
     ...
     H3 = (zt*zt*zt-3*zt)/sqrt(6);
     H4 = (zt*zt*zt*zt-6*zt*zt+3)/sqrt(24);
     phi = 1 + gam1*H3 + gam2*H4;

     fff += log(lam) - 0.5*zt*zt +  2*log(abs(phi));
     theta1_t1 = theta1_t;
     theta2_t1 = theta2_t;
   }
  return fff;
